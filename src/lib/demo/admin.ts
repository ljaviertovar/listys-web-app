...
import { getDemoUserEmail, getDemoUserId, getDemoUserPassword, isDemoModeEnabled } from '@/lib/demo/config'
...
async function ensureDemoUser(admin: AdminClient) {
  ...
  const configuredUserId = getDemoUserId()
  ...
  if (existingUser) {
    if (configuredUserId && existingUser.id !== configuredUserId) {
      throw new Error(`Configured DEMO_USER_ID (${configuredUserId}) does not match the existing demo user (${existingUser.id}).`)
    }
    ...
    if (updateError || !updatedUser.user) {
      throw new Error(`Failed to update demo user: ${updateError?.message || 'Unknown error'}`)
    }
  ...
  if (configuredUserId && createdUser.user.id !== configuredUserId) {
    throw new Error(
      `Configured DEMO_USER_ID (${configuredUserId}) does not match the newly created demo user (${createdUser.user.id}).`
    )
  }
...
async function purgeDemoUserData(admin: AdminClient, userId: string) {
  const [
    { data: groups },
    { data: baseLists },
    { data: tickets },
