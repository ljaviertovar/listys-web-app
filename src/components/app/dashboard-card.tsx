...
function DashboardCardContent({
  ...
  return (
    <Card
      variant="premium"
      className="group relative cursor-pointer overflow-hidden transition-all duration-500  hover:shadow-2xl hover:shadow-primary/10 bg-card"
    >
      <Link href={href} className="flex h-full flex-col">
        {/* Premium Background Effects */}

        <div className="hero-mesh absolute inset-0 opacity-60 transition-opacity duration-500 group-hover:opacity-80" />
        ...
        <CardHeader className="relative z-20 space-y-4 pb-2">
          ...
          <div className="space-y-1.5">
            ...
            <CardDescription className="text-xs font-medium leading-relaxed text-muted-foreground/80 line-clamp-2">
              {description}
            </CardDescription>
          </div>
        </CardHeader>
        ...
        <CardContent className="relative z-20 mt-auto pt-4">
          <div className="flex items-baseline justify-between border-t border-border/40 pt-2">
            ...
            <div className="mb-1 flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-xs font-bold uppercase tracking-wider text-primary transition-all duration-300 group-hover:bg-primary/20 group-hover:scale-105">
              <span>View all</span>
              <HugeiconsIcon
